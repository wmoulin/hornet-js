...
describe('SimpleAction', () => {
    ...
    it('should not call dispatcher', function (doneFn) {
        var promise = new simpleAction(simpleAction.CHANGE_PAGE_COMPONENT).withContext(<ActionContext>{}).withPayload('payload').promise(new ActionsChainData());
        promise.then(() => {
    ...
    it('should call dispatcher', () => {
        ...
        // Act
        return new simpleAction(simpleAction.CHANGE_PAGE_COMPONENT).withContext(<any>actionContext).withPayload(newComponent).promise(new ActionsChainData()).then(() => {
            // Assert
